...
// ===================== LOCAL STORAGE =====================
function loadState() { /* Reads stored data from LocalStorage
Converts it from JSON format to JavaScript object
Restores it into the state variable*/
  try {
...
function saveState() { /*Converts the state object into JSON text
...
// ===================== INITIALISATION =====================
function init() {/*Loads saved data from browser storage
Sets today's date
Renders all UI sections
Adds event listeners for buttons and inputs
Opens the name input modal if the user name is empty*/
  loadState();
  setTodayDate();
  renderAll();
  setupEventListeners();
